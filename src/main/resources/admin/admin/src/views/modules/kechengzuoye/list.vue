		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('kechengzuoye','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia10" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","fontWeight":"600","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kechengzuoye','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- xiangguanfujian -->
					<!-- $column.hiden -->
					<el-table-column :resizable='true' :sortable='true' prop="xiangguanfujian" label="相关附件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.xiangguanfujian" type="text" size="small" @click="download(scope.row.xiangguanfujian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<!-- jiaoshigonghao -->
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kechengzuoye','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('kechengzuoye','提交作业')" @click="zuoyetijiaoCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#212529","height":"40px"}'></span>
								提交作业
							</el-button>
							<el-button class="edit" v-if=" isAuth('kechengzuoye','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai17" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('kechengzuoye','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('kechengzuoye','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import zuoyetijiaoCrossAddOrUpdate from "../zuoyetijiao/add-or-update";
	...
	this.$router.push({path:'/discusskechengzuoye',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	kechengzuoyestatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kechengzuoye/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
