...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="课程名称" prop="kechengmingcheng">
            <el-input v-model="ruleForm.kechengmingcheng" 
                placeholder="课程名称" clearable :disabled=" false  ||ro.kechengmingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="作业名称" prop="zuoyemingcheng">
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="相关附件" prop="xiangguanfujian">
            <file-upload
            tip="点击上传相关附件"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="教师工号" prop="jiaoshigonghao">
          ...
          <el-form-item :style='{"width":"49%","padding":"10px","margin":"0 0 10px","background":"none"}' label="作业内容" prop="zuoyeneirong">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`kechengzuoye/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('kechengzuoye/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`kechengzuoye/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      xiangguanfujianUploadChange(fileUrls) {
          this.ruleForm.xiangguanfujian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
